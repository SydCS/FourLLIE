...
class enhancement_model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_H, self.fake_Amp, self.fake_H_s1, self.snr = self.netG(
            self.var_L
        )  # forward
        ...
        l_pix = self.l_pix_w * self.cri_pix(self.fake_H, self.real_H)  # L_s2_1
        # l_pix_amp = self.l_pix_w * self.cri_pix(self.fake_H_s1, self.real_H) * 0.5
        l_amp = self.l_pix_w * self.cri_pix_ill(self.fake_Amp, self.real_Amp) * 0.01  # L_s1   # 0.01 -> 0.6 不行
        # l_pha = self.l_pix_w * self.cri_pix_ill2(self.fake_Pha, self.real_Pha) * 0.01
        l_vgg = self.l_pix_w * self.cri_vgg(self.fake_H, self.real_H) * 0.2  # L_s2_2   # TODO: 0.1 -> 0.2
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        out_dict["rlt"] = self.fake_H.detach()[0].float().cpu()  # stage 2 输出
        out_dict["rlt_s1"] = self.fake_H_s1.detach()[0].float().cpu()  # stage 1 输出
